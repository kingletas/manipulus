...
def write_magento_module(plan: Plan, module_dir: Path, dry_run: bool = False) -> Path:
    """Emit a Magento module that contributes the bundles map.
    ...
    vendor, _, name = MODULE_NAME.partition("_")

    files = {
        "registration.php": (
            "<?php\n\ndeclare(strict_types=1);\n\n"
            "use Magento\\Framework\\Component\\ComponentRegistrar;\n\n"
            f"ComponentRegistrar::register(ComponentRegistrar::MODULE, '{MODULE_NAME}', __DIR__);\n"
        ),
        "etc/module.xml": (
            '<?xml version="1.0"?>\n'
